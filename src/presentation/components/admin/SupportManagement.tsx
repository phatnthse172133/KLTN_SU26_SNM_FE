...
const isFinal = (status: string) => ["Resolved", "Rejected", "Closed"].includes(status);
const displayStatus = (status: string) => status.replace(/([a-z])([A-Z])/g, "$1 $2");
const tone = (status: string) => status === "Resolved" ? "bg-emerald-50 text-emerald-700" : status === "Rejected" ? "bg-red-50 text-red-700" : status === "InProgress" ? "bg-blue-50 text-blue-700" : "bg-indigo-50 text-indigo-700";
...
export function SupportManagement() {
  ...
  const load = useCallback(async () => {
    ...
    try {
      const page = await supportService.getAdmin({ Page: 1, PageSize: 50, Keyword: query.trim() || null, Status: status || null });
      setItems(page.items ?? []);
  ...
  const open = useCallback(async (id: string) => {
    try { setError(""); setSelected(await supportService.getAdminDetail(id)); }
    catch (cause) { setError(getErrorMessage(cause)); }
  }, []);
  ...
  useEffect(() => {
    const refresh = (event: Event) => {
      const type = (event as CustomEvent).detail?.eventType;
      if (!["SupportTicketCreated", "SupportTicketReplied", "SupportTicketStatusChanged", "SupportAttachmentAdded"].includes(type)) return;
      void load(); if (selected?.id) void open(selected.id);
    };
    window.addEventListener("realtime:event", refresh);
    return () => window.removeEventListener("realtime:event", refresh);
  }, [load, open, selected?.id]);
  useOnReconnect(() => { void load(); if (selected?.id) void open(selected.id); });

  const updateStatus = async (next: string) => {
    if (!selected) return;
    const note = next === "Rejected" ? window.prompt("Enter the reason for rejecting this support request:")?.trim() : undefined;
    if (next === "Rejected" && !note) { setError("A rejection reason is required."); return; }
    setSaving(true);
    try { setError(""); setSelected(await supportService.updateAdmin(selected.id, next, note)); await load(); }
    catch (cause) { setError(getErrorMessage(cause)); }
