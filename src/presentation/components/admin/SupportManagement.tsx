...
export function SupportManagement() {
  ...
  const updateStatus = async (next: string) => { if (!selected) return; const note = next === "Rejected" ? window.prompt("Enter the reason for rejecting this support request:", statusNote) : statusNote; if (next === "Rejected" && !note?.trim()) { setError("A rejection reason is required."); return; } setSaving(true); try { setError(""); setSelected(await supportService.updateAdmin(selected.id, next, note?.trim() || undefined)); setStatusNote(""); await load(); } catch (updateError) { setError(getErrorMessage(updateError)); } finally { setSaving(false); } };
