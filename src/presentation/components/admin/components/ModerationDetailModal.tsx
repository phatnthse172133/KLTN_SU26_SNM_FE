...
function SummaryItem({ icon: Icon, label, value, tone }: { icon: LucideIcon; label: string; value: number; tone: keyof typeof SUMMARY_TONES }) {
  return (
    <div className={`rounded-xl p-4 ${SUMMARY_TONES[tone]}`}>
...
export function ModerationDetailModal({ isOpen, onClose, targetId, targetType, onActionClick }: ModerationDetailModalProps) {
  ...
  const lifecycle = isMarket ? mDetail?.lifecycleStatus : bDetail?.zoneName ? 'Allocated' : 'Unallocated';
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/40 backdrop-blur-sm" role="dialog" aria-modal="true" aria-labelledby="modal-title">
      <div className="bg-white rounded-2xl w-full max-w-4xl max-h-[90vh] flex flex-col shadow-2xl overflow-hidden animate-in fade-in zoom-in duration-200">
        ...
        <div className="p-6 border-b border-slate-100 flex items-start gap-4">
          ...
          <div className="flex-1 min-w-0">
            ...
            <div className="flex gap-2 mt-3">
              ...
              {lifecycle && (
                <span className="px-2.5 py-1 text-xs font-bold uppercase tracking-wider bg-slate-100 text-slate-600 rounded-md">
                  {lifecycle}
                </span>
              )}
            </div>
                ...
                <div className="space-y-8 animate-in fade-in duration-300">
                  {!isMarket && bDetail?.nightMarketName && (
                    <div className="flex items-center gap-1.5 text-slate-500 text-sm mt-1">
                      ...
                      {bDetail.zoneName && <span className="opacity-75">â€¢ {bDetail.zoneName}</span>}
                    </div>
                  )}
                  {isSuspended && (
                  ...
                  {isMarket && mDetail && (
                    <>
                      ...
                      <div>
                        <h3 className="text-sm font-bold text-slate-900 uppercase tracking-wider mb-4">Platform Summary</h3>
                        <div className="grid grid-cols-2 lg:grid-cols-4 gap-3">
                          <SummaryItem icon={Store} label="Total Booths" value={mDetail.totalBooths} tone="indigo" />
                          <SummaryItem icon={Store} label="Active Booths" value={mDetail.activeBooths} tone="emerald" />
                          <SummaryItem icon={MessageSquareWarning} label="Complaints" value={mDetail.totalComplaintCount} tone="amber" />
                          <SummaryItem icon={AlertTriangle} label="Serious" value={mDetail.seriousComplaintCount} tone="red" />
                        </div>
                      </div>
                    </>
                  )}
        ...
        {/* Footer */}
        <div className="p-4 border-t border-slate-100 bg-slate-50/50 flex flex-col sm:flex-row items-center justify-between shrink-0 gap-4">
