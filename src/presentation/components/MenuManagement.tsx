...
export function MenuManagement() {
  ...
  const openAdd = () => { setEditingItem(null); setDraft({ ...emptyDraft, categoryId: categories[0]?.id ?? "" }); setImageFile(null); setError(""); setShowForm(true); };
  const openEdit = (item: FoodItem) => { const sale = getSale(item.id); setEditingItem(item); setDraft({ categoryId: item.categoryId, name: item.name, description: item.description ?? "", price: String(item.price), thumbnailUrl: item.thumbnailUrl ?? "", isAvailable: item.isAvailable, isFeatured: item.isFeatured, salePrice: sale ? String(sale.price) : "", saleStart: sale?.startDate?.slice(0,10) ?? "", saleEnd: sale?.endDate?.slice(0,10) ?? "" }); setImageFile(null); setError(""); setShowForm(true); };
  ...
  const saveItem = async () => {
    ...
    try {
      ...
      if (imageFile) {
        try {
          const uploadResponse = await menuService.uploadThumbnail(selectedBooth.id, foodItemId, imageFile);
